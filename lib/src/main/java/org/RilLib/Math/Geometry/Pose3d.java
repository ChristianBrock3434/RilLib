...
import org.RilLib.Math.Numbers.N4;
import org.RilLib.Math.Util.Pose3dUtil;
...
public class Pose3d implements Interpolatable<Pose3d> {
     ...
     *     degrees since the previous pose update, the twist would be Twist3d(0.01, 0.0, 0.0, new
    ...
    public Pose3d exp(Twist3d twist) {
        ...
        // Rotate translation_component by current rotation and add to current translation
    ...
    public Twist3d log(Pose3d end) {
        ...
        double[] u = {transform.getTranslation().getX(), transform.getTranslation().getY(), transform.getTranslation().getZ()};
